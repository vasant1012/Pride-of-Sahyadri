...
class RAGEngine:
    ...
    def build_corpus(self, df):
        ...
        logger.info(
            f"RAGEngine: Corpus created with {len(self.corpus)} entries.")  # NOQA E501
        return self
...
if __name__ == "__main__":
    query = input("Enter your question about Maharashtra forts: ")
    # query = "Tell me about fort Sindhudurg"
    df = pd.read_csv(
        "/home/vasant/projects/Pride-of-Sahyadri/data/maharashtra-forts.csv")
    rag = RAGEngine()
    rag.build_corpus(df=df)
    rag.build_index()
